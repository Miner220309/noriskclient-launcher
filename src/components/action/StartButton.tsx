...
export const StartButton = (props: Props) => {
  const checkForInstalledNatives = async (mcDir: string, os: string) => {
    ...
    let hasNatives = await promisified<boolean>({
      cmd: "fileExists",
      path: lwjglFolder,
    });
    if (hasNatives) {
  ...
  const createNRCFolder = async (mcDir: string, os: string) => {
    ...
    await createDir(mcDir + "/norisk/natives")
      .then(() => {
  ...
  const startGame = async () => {
    ...
    const mcDir = await promisified<string>({ cmd: "minecraftDir" }).then(
      (value) => {
    ...
    const mcJson = JSON.parse(
      await readTextFile(mcDir + version.jsonPath, {})
    ) as MCJson;
    let libraries = "";
    mcJson.libraries.forEach((value) => {
      const path = value.name
        .substr(0, value.name.indexOf(":"))
        .replaceAll(".", "/");
      const jarName = value.name
        .substr(value.name.indexOf(":") + 1, value.name.length)
        .substr(
          0,
          value.name
            .substr(value.name.indexOf(":") + 1, value.name.length)
      ...
      const versionNumber = value.name.substr(
      ...
      libraries =
        ...
        (mcDir +
          ...
          path +
          value.name
            .substr(value.name.indexOf(":"), value.name.length)
            .replaceAll(":", "/") +
          "/" +
          jarName +
          "-" +
    ...
    invoke({
      ...
      args: [
        ...
        version.assetIndex,
        `--uuid`,
        profile.minecraftProfile.id,
        `--accessToken`,
        profile.accessToken,
        `--userProperties`,
        profile.userProperites.length === 0
          ? ""
          : profile.userProperites.length,
        `--userType`,
        profile.type,
        `${version.tweakClass}`,
      ],
